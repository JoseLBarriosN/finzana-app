...
const database = {
    ...
    verificarElegibilidadCliente: async (curp, office) => {
        try {
            ...
            // --- CASO A: SIN DEUDAS ACTIVAS (Puede ser Nuevo, Reingreso O RENOVACIÓN RECIENTE) ---
            if (creditos.length === 0) {
                // Buscamos el historial más reciente (el que acabamos de liquidar)
                ...
                if (office && office !== 'AMBAS') {
                    queryHistorial = queryHistorial.where('office', '==', office);
                }
                
                const historialSnap = await queryHistorial.get();
                
                if (!historialSnap.empty) {
                    const ultimoCredito = { id: historialSnap.docs[0].id, ...historialSnap.docs[0].data() };
                    const histId = ultimoCredito.historicalIdCredito || ultimoCredito.id;
    
                    // BUSCAMOS SI EL ÚLTIMO PAGO FUE EL GATILLO DE RENOVACIÓN
                    const pagosSnap = await db.collection('pagos')
                        .where('idCredito', '==', histId)
                        .where('office', '==', (ultimoCredito.office || office))
                        .orderBy('fecha', 'desc')
                        .limit(1) // Solo el último pago importa
                        .get();
    
                    let esGatilloRenovacion = false;
                    if (!pagosSnap.empty) {
                        const ultimoPago = pagosSnap.docs[0].data();
                        // Si el pago que liquidó fue "actualizado" o "renovacion"
                        if (ultimoPago.tipoPago === 'actualizado' || ultimoPago.tipoPago === 'renovacion') {
                            esGatilloRenovacion = true;
                        }
                    }
    
                    if (esGatilloRenovacion) {
                        // ¡EUREKA! No es nuevo, es una renovación en proceso
                        return { 
                            elegible: true, 
                            mensaje: `Renovación detectada (Crédito anterior liquidado).`, 
                            esRenovacion: true,
                            esReingreso: false,
                            datosCreditoAnterior: ultimoCredito,
                            triggerRenovacion: true // Bandera especial
                        };
                    } else {
                        // Si liquidó normal hace tiempo, es Reingreso
                        return { 
                            elegible: true, 
            ...
            if (esComisionista) {
                if (creditos.length >= 2) {
                    if (puedeRenovar) {
                        return { 
                            elegible: true, 
                            mensaje: `Comisionista (2 créditos). Elegible para RENOVACIÓN.`,
                            esRenovacion: true,
                            datosCreditoAnterior: creditoActual
                        };
                    } else {
                        return { elegible: false, mensaje: `Comisionista al límite. Crédito actual: ${semanasPagadas} sem (Req: 10).` };
                    }
                } else {
                    return {
                        elegible: true,
                        mensaje: `Comisionista (1 crédito). Puede pedir SEGUNDO o RENOVAR.`,
            ...
            if (puedeRenovar) {
                return { 
                    elegible: true, 
                    mensaje: `Cliente regular (${semanasPagadas} sem). Elegible para RENOVACIÓN.`, 
                    esRenovacion: true,
                    datosCreditoAnterior: creditoActual,
                    forzarRenovacion: true
                };
            } else {
                return { elegible: false, mensaje: `Crédito activo (${semanasPagadas} sem). Se requieren 10 para renovar.` };
            }
    ...
    verificarElegibilidadAval: async (curpAval, office) => {
        ...
        try {
            ...
            for (const credito of creditosAvalados) {
                ...
                let saldoActual = credito.saldo !== undefined ? credito.saldo : credito.montoTotal;
                const montoTotal = credito.montoTotal || 0;

                // 2. VERIFICACIÓN EN TIEMPO REAL (Corrección del error 1303)
                // Si el saldo dice que debe, consultamos los pagos reales para estar 100% seguros
                // de que no es un error de sincronización de la base de datos.
                if (saldoActual > 0) {
                    const historialId = credito.historicalIdCredito || credito.id;
                    const pagosSnap = await db.collection('pagos')
                        .where('idCredito', '==', historialId)
                        .where('office', '==', credito.office) // Asegurar misma oficina
                        .get();
                    
                    const totalPagadoReal = pagosSnap.docs.reduce((sum, doc) => sum + (doc.data().monto || 0), 0);
                    
                    // Recalculamos el saldo real con los pagos encontrados
                    saldoActual = montoTotal - totalPagadoReal;
                    
                    // Ajuste por tolerancia de decimales
                    if (saldoActual < 0.05) saldoActual = 0;
                }

                // Si después de verificar los pagos reales, el saldo es 0, este crédito cuenta como liquidado.
                if (saldoActual === 0) continue; 

                // 3. Cálculo del porcentaje pagado
                ...
                // Si el saldo real > 0, validamos que no esté en mal estado
    ...
    try {
        ...
        const nombrePoblacion = cliente.poblacion_grupo || cliente.poblacion || 'Sin Población';
        ...
        let nuevoCreditoData = {
            ...
            poblacion_grupo: nombrePoblacion,
            ruta: cliente.ruta,
            estado: 'al corriente',
        ...
        let idAgenteAnteriorOwner = auth.currentUser.uid;
        ...
        if (esRenovacion) {
            ...
            if (!creditosAnteriores.empty) {
                ...
                if (oldData.creadoPorId) idAgenteAnteriorOwner = oldData.creadoPorId;
        ...
        if (montoReferenciaRenovacion > 0) {
            descCaja += ` (Con Renovación Cr. ${idCreditoAnteriorHist})`;
        }
        ...
        if (montoReferenciaRenovacion > 0 && crearPagoLiquidacionAutomatico) {
            ...
            batch.set(movEntradaRef, {
                ...
                categoria: 'COBRANZA',   
                monto: Math.abs(montoReferenciaRenovacion), 
        ...
        if (!esCreditoComisionista && generaComisionApertura) {
            ...
            batch.set(comisionRef, {
                ...
                categoria: 'COMISION',
                monto: -100,
                descripcion: `Comisión apertura ${tipoCredito.toUpperCase()} ${cliente.nombre} (${nuevoFolio})`,
                creditoId: nuevoCreditoRef.id,
                poblacion: nombrePoblacion, // <--- Correcta población
                registradoPor: userEmail,
                office: office
   ...
   async agregarPago(pagoData, emailUsuario, firestoreIdCredito) {
    try {
        ...
        // CORRECCIÓN POBLACIÓN: Usamos un fallback seguro
        // El reporte agrupa por el campo 'poblacion'.
        const nombrePoblacion = credito.poblacion_grupo || credito.poblacion || 'Sin Población';

        const idAgenteOwner = credito.creadoPorId || auth.currentUser.uid;
        ...
        const nuevoPago = {
            ...
            syncStatus: 'pending',
            // GUARDAMOS AMBOS CAMPOS PARA ASEGURAR COMPATIBILIDAD
            poblacion_grupo: nombrePoblacion, 
            poblacion: nombrePoblacion, // <--- Esto arregla la Hoja de Corte
            ruta: credito.ruta || ''
        };
        ...
        // 3. REGISTRAR COMISIÓN (SALIDA)
        // NOTA: NO registramos el ingreso aquí porque la Hoja de Corte lee la colección 'pagos'.
        // Al guardar 'nuevoPago' arriba con el campo 'poblacion', ya aparecerá en el lugar correcto.
        ...
        if (!isNaN(comisionVal) && comisionVal > 0) {
            ...
            console.log(`✅ Comisión (-$${comisionVal}) registrada en población: ${nombrePoblacion}`);
        }
    ...
    obtenerReporteMulticreditos: async (office) => {
        try {
            console.log(`🔎 Auditoría Multicréditos (Cálculo Real + Info Cliente) en ${office}...`);
            ...
            // 3. Filtrar clientes y validar saldos
            ...
            for (const [curp, listaCreditos] of mapaClientes.entries()) {
                if (listaCreditos.length > 1) {
                    ...
                    // Validar saldos reales con pagos
                    ...
                    if (creditosRealmenteActivos.length > 1) {
                        ...
                        // --- NUEVO: OBTENER SI ES COMISIONISTA ---
                        // Consultamos el cliente para tener el dato veraz
                        let esComisionista = false;
                        try {
                            ...
                            if (!clienteSnap.empty) {
                                esComisionista = clienteSnap.docs[0].data().isComisionista === true;
                            }
                        } catch (e) { console.error("Error buscando info comisionista", e); }
                        ...
                        clientesProblema.push({
                            ...
                            esComisionista: esComisionista // <--- Dato Agregado
                        });
                    }
                }
            }
